...
;; this file generated by convert-soft-contract.rkt using ex-13.sch as input
  ...
  (λ ()
    (define (_f x y)
      (cond
       ...
       (else (number? x))))
    (define-id-with-ctc (-> any/c any/c (not/c false?)) _f f bug bug)
    (f
     (apply-ctc
      any/c
...
(define (counterexample)
  (define test-result (concolic-test ex-13 #:all? #f #:statistics? #t))
  (define witness (list-ref test-result 0))
  (define stats (list-ref test-result 1))
  (vector
   (concretize-input ex-13 witness)
   '()
...
(provide counterexample (rename-out (ex-13 test-property)))
