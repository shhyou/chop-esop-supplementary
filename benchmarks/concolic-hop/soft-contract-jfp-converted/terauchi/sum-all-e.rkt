...
;; this file generated by convert-soft-contract.rkt using sum-all-e.sch as input
...
(define-concolic-test
 ...
 (prop-not-exn
  (λ ()
    ...
    (define (_sum x) (if (<= x 0) 0 (+ x (_sum (- x 1)))))
    (define (_h y) (assert (<= (+ y y) (_sum y))))
    (define (_main n) (_h n))
...
(define (counterexample)
  (define test-result (concolic-test sum-all-e #:all? #f #:statistics? #t))
  (define witness (list-ref test-result 0))
  (define stats (list-ref test-result 1))
  (vector
   (concretize-input sum-all-e witness)
   '()
...
(provide counterexample (rename-out (sum-all-e test-property)))
