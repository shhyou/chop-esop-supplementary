...
;; this file generated by convert-soft-contract.rkt using foldr1.sch as input
  ...
  (λ ()
    (define (_foldr1 f xs)
      (let ((z (car xs)) (zs (cdr xs)))
        (if (empty? zs) z (f z (_foldr1 f zs)))))
    (define-id-with-ctc
...
(define (counterexample)
  (define test-result (concolic-test foldr1 #:all? #f #:statistics? #t))
  (define witness (list-ref test-result 0))
  (define stats (list-ref test-result 1))
  (vector
   (concretize-input foldr1 witness)
   '()
...
(provide counterexample (rename-out (foldr1 test-property)))
